...
typedef struct frame{
    int frame_kind; //ACK:0, SEQ:1(data) FIN:2(im done sending the data)
    int sq_no;
    int ack;
    Packet packet;
}Frame;
...
int main(int argc, char** argv){
	...
	while(1){
		...
		if (f_recv_size > 0) {
		    if (frame_recv.frame_kind == 2) { // FIN received
			printf("[+] FIN frame received. Exiting.\n");
			break;
		    }
